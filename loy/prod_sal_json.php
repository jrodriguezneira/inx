...
{ 
  ...
  $posts['sku']=$row['sku'];
  $posts['name']=$row['name'];
  $posts['day_sales']=$row['day_sales'];

  //Get sales from previous day
  $sql1="SELECT day_sales FROM product_sales  where sku='".$row['sku']."' and date_report=DATE_SUB(CURDATE(), INTERVAL 3 DAY);";
  ...
  $sql2="SELECT day_sales FROM product_sales  where sku='".$row['sku']."' and date_report=DATE_SUB(CURDATE(), INTERVAL 4 DAY);";
